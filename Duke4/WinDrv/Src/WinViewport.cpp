...
void UWindowsViewport::OpenWindow( DWORD InParentWindow, UBOOL IsTemporary, INT NewX, INT NewY, INT OpenX, INT OpenY )
{
	...
	if(!RenDev)
	{
		appMsgf(TEXT("Could not set render device ... attempting to revert to software, please report this to Nick. (%ix%i), Fullscreen:%i, ColorBytes:%i"),NewX,NewY,(!GIsEditor && GetOuterUWindowsClient()->StartupFullscreen)?1:0,IsTemporary?ColorBytes:INDEX_NONE );
	...
	if(DoSetActive)	SetActiveWindow( Window->hWnd );
}
...
/*
	...
	{
		HRESULT Result;
		INT DX=0, DY=0, DZ=0, LDown, RDown, MDown;
		...
		{
			// Peel off relevant events.
			switch ( diMouseBuffer[i].dwOfs )
			{
				case DIMOFS_BUTTON0:
				...
				case DIMOFS_BUTTON2:
					...
					break;
				case DIMOFS_X:
					DX += diMouseBuffer[i].dwData;
					break;
				case DIMOFS_Y:
					DY += diMouseBuffer[i].dwData;
					break;
				case DIMOFS_Z:
					DZ = diMouseBuffer[i].dwData;
					break;
			}

			// Dispatch mouse wheel messages.
			if ( DZ < 0)
			{
		...
		// Dispatch accumulated axial motion.
	...
	*/
	...
	if( diMouse && AcquiredDIMouse )
	{
		...
		LDown = diMouseState.rgbButtons[0] & 0x80;
		RDown = diMouseState.rgbButtons[1] & 0x80;
		MDown = diMouseState.rgbButtons[2] & 0x80;
		DX += diMouseState.lX;
		DY += diMouseState.lY;
		DZ = diMouseState.lZ;
...
void UWindowsViewport::AcquireDIMouse( UBOOL Capture )
{
	...
	if(Capture)	diMouse->Acquire();
	else		diMouse->Unacquire();
}
...
UBOOL UWindowsViewport::diSetupKeyboardMouse()
{
	...
	// Setup keyboard.
	hr = CoCreateInstance( CLSID_DirectInputDevice8, NULL, CLSCTX_INPROC_SERVER, IID_IDirectInputDevice8, (void**) &diKeyboard );
	if ( FAILED( hr ) )
	{
		appErrorf( NAME_Init, TEXT("Failed to create DirectInput keyboard device. %x"), hr );
		return false;
	}
	hr = diKeyboard->Initialize( hInstance, DIRECTINPUT_VERSION, GUID_SysKeyboard );
	if ( FAILED( hr ) )
	{
		appErrorf( NAME_Init, TEXT("Failed to initialize DirectInput keyboard device.") );
		return false;
	}
	hr = diKeyboard->SetDataFormat( &c_dfDIKeyboard );
	if ( FAILED( hr ) )
	{
		appErrorf( NAME_Init, TEXT("Failed to set DirectInput keyboard device data format.") );
		return false;
	}
	hr = diKeyboard->SetCooperativeLevel( Window->hWnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE );
	if ( FAILED( hr ) )
	{
		appErrorf( NAME_Init, TEXT("Failed to set DirectInput keyboard device cooperative level.") );
		return false;
	}

	// Setup mouse.
	hr = CoCreateInstance( CLSID_DirectInputDevice8, NULL, CLSCTX_INPROC_SERVER, IID_IDirectInputDevice8, (void**) &diMouse );
	if ( FAILED( hr ) )
	{
		appErrorf( NAME_Init, TEXT("Failed to create DirectInput mouse device.") );
		return false;
	}
	hr = diMouse->Initialize( hInstance, DIRECTINPUT_VERSION, GUID_SysMouse );
	if ( FAILED( hr ) )
	{
		appErrorf( NAME_Init, TEXT("Failed to initialize DirectInput mouse device.") );
		return false;
	}
	...
	dipdw.dwData			= 16;
	hr = diMouse->SetProperty( DIPROP_BUFFERSIZE, &dipdw.diph );
	 *
	 */
